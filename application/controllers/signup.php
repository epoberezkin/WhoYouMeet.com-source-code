...
class Signup extends CI_Controller {
	...
	public function index() { // signup
		if ( $this->session->userdata('logged_in') ) {
			redirect('/');
				...
				array(
					'content' => 'signup', // view_signup.php is loaded
					'title' => my_lang('page_signup_title')
					));
	}
	...
	public function validate() {
		$this->form_validation->set_rules('fullname', my_lang('form_signup_fullname_field'),
			'required|trim|xss_clean');
		$this->form_validation->set_rules('email', my_lang('form_signup_email_field'),
			'required|trim|valid_email|is_unique[users.email]|xss_clean');
		$this->form_validation->set_rules('password', my_lang('form_signup_password_field'), 'required|matches[c_password]|trim|xss_clean');
		$this->form_validation->set_rules('c_password', my_lang('form_signup_c_password_field'), 'required|trim|xss_clean');

		$this->form_validation->set_message('is_unique', my_lang('form_signup_another_user_email'));
		...
		if ($this->form_validation->run()) {
			...
			if ($user){
				...
				if ( $this->send_verification_email($user, $key) ){
					...
					$this->session->set_flashdata('success', my_lang('msg_success_signup_ok'));
					redirect('/');
				} else {
					// email not sent
					$this->session->set_flashdata('success', my_lang('msg_success_signup_no_email'));
					$this->session->set_flashdata('error', my_lang('msg_error_signup_email_not_sent'));
					redirect('/');
				}
			} else {
				 ...
				 */
				$this->session->set_flashdata('error', my_lang('msg_error_signup_database'));
	...
	public function verify_email($key) {
		...
		if ( $this->model_users->verify_user($key) ) {
			// User verified
			$this->session->set_flashdata('success', my_lang('msg_success_verification_ok'));
			if ( $this->session->userdata('logged_in') ) {
				redirect('/');
			} else {
				redirect('/login');
			...
			if ( $this->session->userdata('logged_in') ) {
				...
				if ($user->verified) {
					...
					$this->session->set_flashdata('info', my_lang('msg_info_already_verified'));
					redirect('/');
				} else {
					...
					if ($user->email) {
						...
						if ( $this->send_verification_email($user, $key) ){
							// email re-sent
							$this->session->set_flashdata('alert', my_lang('msg_alert_cant_verify_key_expired_email_resent'));
							redirect('/');
						} else {
							// email not re-sent
							$this->session->set_flashdata('error', my_lang('msg_error_cant_verify_key_expired_cant_resend'));
							redirect('/');
						}
					} else {
						// We don't even have email
						$this->session->set_flashdata('alert', my_lang('msg_alert_cant_verify_no_email'));
						redirect('i/profile/edit');
					}
				}
			} else {
				// the key is invalid and user is not logged in - so we have no idea who he is
				$this->session->set_flashdata('alert', my_lang('msg_alert_cant_verify_key_expired_logged_out'));
