 ...
 *
 * List of functions:
 *  1. public function index() - opens login form
 *  2. public function validate() - validates login form and logs in user
 *  3. function login_credentials() - call-back function for email validation, does actual login (via model_users.php)
 *  4. public function social_redirect($social_network = '') - redirects to social network for authentication
 *  4a.public function facebook_redirect() - redirects to facebook.com for authentication
 *  4b.public function twitter_redirect() - redirects to twitter.com for authentication
 *  4c.public function linkedin_redirect() - redirects to linkedin.com for authentication
 *  5. public function social($social_network = '') - redirect "processor" for social login/signup, called by facebook(), twitter(), linkedin()
 *  5a.public function facebook() - redirect point from facebook, does actual facebook login/signup
 *  5b.public function twitter() - redirect point from twitter, does actual twitter login/signup
 *  5c.public function linkedin() - redirect point from linkedin, does actual twitter login/signup
 *  6. public function logout() - logs out user
 *  7. public function recover($action = '', $param = '') - recovers forgotten user password.
 *  8. function validate_recover_password_email($email) - call-back function for email field validation for password recovery form
 *  9. function send_recovery_email($user, $key) - sends recovery email
 * 10. public function recover_password($action = '') - change password during password recovery
 * 
 */
...
class Login extends CI_Controller {
	...
	public function index() { // login
		if ( $this->session->userdata('logged_in') ) {
			redirect('/');
		} else $this->load->view('includes/view_template',
				array(
					'content' => 'login',  // view_login.php is loaded
					'title' => my_lang('page_login_title')
				));
	}
	...
	public function validate() {
		$this->form_validation->set_rules('email', my_lang('form_login_email_field'), 'required|trim|valid_email|xss_clean|callback_login_credentials'); /* login_credentials() will be called */
		$this->form_validation->set_rules('password', my_lang('form_login_password_field'), 'required|trim|xss_clean');
	...
	function login_credentials($email) {
		...
		if ($user) {
			...
			$this->form_validation->set_message('login_credentials', my_lang('msg_alert_login_no_user'));
			return false;
		}
	}
	...
	public function social_redirect($social_network = '') {
		...
		case 'twitter':
			...
			// function twitter() will be called after authentication
			$ok = $this->twconnect->twredirect('/login/twitter');
			break;

		case 'linkedin':
			$this->load->library('in_connect');

			// function linkedin() will be called after authentication
			$ok = $this->in_connect->in_redirect('/login/linkedin');
			break;
		...
		if ( ! $ok) {
			$this->session->set_flashdata('alert', my_lang('msg_alert_social_cant_connect', $social_network));
			redirect($previous_page);
		}
	}
	...
	/**
	 * 4a. Redirects to twitter.com for authentication
	 */
	public function twitter_redirect() {
		$this->social_redirect('Twitter');
	}


	/**
	 * 4b. Redirects to linkedin.com for authentication
	 */
	public function linkedin_redirect() {
		$this->social_redirect('LinkedIn');
	}
	...
	public function social($social_network = '') {
		...
		case 'twitter':
			...
			if ($ok) {
				$this->load->model('model_users');
				// both login and signup happen in this function
				$user = $this->model_users->login_twitter_user( $this->twconnect->tw_user_id );
			}
			break;
		...
		case 'linkedin':
			...
			if ($ok) {
				$this->load->model('model_users');
				// both login and signup happen in this function
				$user = $this->model_users->login_linkedin_user( $this->in_connect->in_user,  $this->in_connect->in_access_token );
			}
			break;
		...
		case 'facebook':
			...
			if ($fb_user) {
				$ok = true;
				$this->load->model('model_users');
				$user = $this->model_users->login_facebook_user($fb_user);
			} else {
				$ok = $user = false;
			}
		...
		if ( $ok && $user ) {
			...
			$this->session->set_flashdata('error', my_lang('msg_error_login_social'));
			...
			$this->session->set_flashdata('alert', my_lang('msg_alert_social_connect_declined', $social_network));
	...
	/**
	 * 5b. Redirect point from twitter, does actual twitter login/signup
	 */
	...
	/**
	 * 5c. Redirect point from linkedin, does actual twitter login/signup
	 */
	...
	/**
	 * 7. Recovers forgotten user password.
	 *    $action:
	 *      'form' - form to request recovery message
	 *      'validate' - validate email and send recovery message
	 *		'reset' - reset key when user did not request recovery
	 *      'key' - process recovery based on the key
	 */
		...
		if ( !$this->session->userdata('logged_in') ) {
			...
			case '':
				...
				$this->load->view('includes/view_template',
					array(
						'content' => 'recover',  // view_recover.php is loaded
						'title' => my_page_title('page_recover_passwd_title'),
			...
			case 'validate':
				...
				$this->form_validation->set_rules('email', my_lang('form_login_email_field'), 'required|trim|valid_email|xss_clean|callback_validate_recover_password_email');
				...
				if ($this->form_validation->run()) {
					...
					$key = $this->model_users->unique_key($user->id);
					if ( $this->send_recovery_email($user, $key) ){
						// recovery email sent
						$this->session->set_flashdata('success', my_lang('msg_success_recovery_msg_sent', $user->email));
						redirect('/');
					} else {
						// recovery email not sent
						$this->session->set_flashdata('error', my_lang('msg_error_cant_send_recovery_msg'));
			...
			case 'key':
				...
				if ( $user_id ) {
					...
					$this->session->set_flashdata('alert', my_lang('msg_alert_cant_recover_key_expired'));
			...
			// user logged in
			$this->session->set_flashdata('alert', my_lang('msg_alert_cant_recover_signed_up'));
			redirect('/');
		}
	}
	...
	function validate_recover_password_email($email) {
		...
		if ( $this->model_users->check_email($email) ) {
			...
			$this->form_validation->set_message('validate_recover_password_email', my_lang('form_recover_passwd_no_such_email'));
			return false;
		}
	}
	...
	function send_recovery_email($user, $key) {
		...
		$message .= my_lang('email_send_recovery_text');
		...
		$message .= my_lang('email_send_recovery_thanks');
	...
	public function recover_password($action = '') {
		if ($this->session->userdata('password_recovery')
			...
			case '':
				$this->load->view('includes/view_template',
					array(
						'content' => 'recover_password',  // view_recover_password.php is loaded
